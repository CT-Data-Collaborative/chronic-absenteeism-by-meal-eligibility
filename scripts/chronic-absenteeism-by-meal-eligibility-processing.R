...
data_location <- grep("Meal", sub_folders, value=T)
...
for (i in 1:length(chronic_absent_dist_noTrend)) {
  ...
  # Choose correct row for column headers
...
backfill_years <- expand.grid(
  ...
  `Free/Reduced Price Meal Eligibility` = unique(chronic_absent_dist$`Free/Reduced Price Meal Eligibility`),
  `Year` = years 
)
...
complete_chronic_absent <- merge(chronic_absent_fips, backfill_years, by = c("FixedDistrict", "Free/Reduced Price Meal Eligibility", "Year"), all=T)

complete_chronic_absent <- complete_chronic_absent[order(complete_chronic_absent$FixedDistrict, complete_chronic_absent$Year, complete_chronic_absent$`Free/Reduced Price Meal Eligibility`),]
...
test <- complete_chronic_absent_long[,c("District", "Year", "Free/Reduced Price Meal Eligibility")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_chronic_absent_long,
  file.path(path_to_top_level, "data", "chronic_absenteeism_by_meal_eligibility_2012-2020.csv"),
  sep = ",",
  row.names = F
)